layout: draft
title: GitHub Pages + Hexo搭建自己的网站
comment: true
tags: [技术,Hexo,GitHub Page,Git]
date: 2016-5-28 22:40:50
updated: 2016-5-30 20:57:00
---

------
# 一、 前言
这是一篇使用GitHub Pages和Hexo搭建网站的总结。

作为一个前端小白，虽说是个码农，但是一开始照着网上的各种教程去搭建的时候，还是云里雾里的，耗费不少时间。
...
### 3.1 使用默认theme
我们继续使用上面的文件夹Hexo（也可以新建一个文件夹重新生成），然后编辑该文件夹下的_config.yml。
...
1. 创建仓库，jilyjiang.github.io；
2. 创建两个分支：master 与 hexo_bakup；
3. 设置hexo为默认分支（因为我们只需要手动管理这个分支上的Hexo网站文件）；
4. 使用git clone git@github.com:JilyJiang/jilyjiang.github.io.git拷贝仓库；
5. 在本地jilyjiang.github.io文件夹下通过Git依次执行npm install hexo、hexo init、npm install 和 npm install hexo-deployer-git（此时当前分支应显示为hexo_bakup）;
6. 修改_config.yml中的deploy参数，分支应为master；
7. 依次执行git add .、git commit -m "..."、git push origin hexo提交网站相关的文件；
8. 执行hexo generate -d生成网站并部署到GitHub上。

这样一来，在GitHub上的jilyjiang.github.io仓库就有两个分支，一个hexo_bakup分支用来存放网站的原始文件，一个master分支用来存放生成的静态网页。

### 4.3 我的博客管理流程
#### 4.3.1 日常修改
在本地对博客进行修改（添加新博文、修改样式等等）后，通过下面的流程进行管理：

1. 依次执行git add .、git commit -m "..."、git push origin hexo指令将改动推送到GitHub（此时当前分支应为hexo_bakup）；
...
1. 使用git clone git@github.com:jilyjiang/jilyjiang.github.io.git拷贝仓库；
2. 在本地新拷贝的jilyjiang.github.io文件夹下通过Git依次执行下列指令：npm install hexo、npm install、npm install hexo-deployer-git（记得，不需要hexo init这条指令）。

## 五、结尾
感谢google的搜索及各路大神的文章参考，最终搞定！O(∩_∩)O哈哈~
