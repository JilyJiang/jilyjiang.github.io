...
有一件事是不同的这里所做的是我们如何填充列表项图像。 在这里，我们得到的URL（＃8），用于此列表项图像，并同时通过网址并向ImageView的列表项的参考ImageDownloader通过调用其下载方法类（＃9）。 此方法将异步加载数据，图像，一旦它完成更新图像。 这是在一个被动的方式进行，因为getView当列表项需要显示将只被调用，因此，只有当一个列表项所示的图像只被检索并在需要的图像。

主要目的ImageDownloader是异步加载图像，但它在内存中还缓存图像。 我们ImageDownloader类有向它提出的，所以我们可以添加缓存图像到磁盘的可能性，也因此我们可以添加我们展示列表中的初始图像和最终下载的图像之间的过渡交叉淡入淡出了一些调整。
